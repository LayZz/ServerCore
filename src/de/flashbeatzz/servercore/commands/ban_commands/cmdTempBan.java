    ...
    public boolean onCommand(CommandSender cs, Command cmd, String s, String[] args) {
        ...
        if(args.length >= 2) {
            ...
            if(cs instanceof Player) {
                ServerCore.sendMessage(SocketTarget.BUNGEECORD, "TEMPBAN", ((Player) cs).getUniqueId() + "<>" + Bukkit.getServer().getPlayer(args[1]).getUniqueId() + "<>" + args[2] + "<>" + reason);
                return true;
            }
            ServerCore.sendMessage(SocketTarget.BUNGEECORD, "TEMPBAN", UUIDLibrary.getConsoleUUID() + "<>" + Bukkit.getServer().getPlayer(args[1]).getUniqueId() + "<>" + args[2] + "<>" + reason);
            return true;
        } else {
